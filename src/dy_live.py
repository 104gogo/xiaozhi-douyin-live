...
def unPackWebcastChatMessage(data):
    ...
    user_name = data.get('user', {}).get('nickName', '未知用户') if data else '未知用户'
    print(f"🎯 收到新弹幕#{GlobalVal.commit_num}: [{user_name}] {content}")
    
    # 更新最新的普通消息到全局变量
    GlobalVal.update_chat_message(data)
...
def unPackWebcastGiftMessage(data):
    ...
    # 更新最新的礼物消息到全局变量
    GlobalVal.update_gift_message(data)
...
def unPackWebcastMemberMessage(data):
    ...
    # 更新最新的成员进入消息到全局变量
    GlobalVal.update_member_message(data)
...
def unPackWebcastLikeMessage(data):
    ...
    # 更新最新的点赞消息到全局变量
    GlobalVal.update_like_message(data)
